...
# Importing random numbers and a random letter from the generator
...
def display():
    ...
    if shortest_paths is not None:
        ...
        for i in range(len(cities)):
            ...
            path_str = ' -> '.join(path)  # Concatenate city names into a path string
            if i == start_city_idx:
...
bellman_time = end_time - start_time
print(f"\nTime taken for Bellman-Ford algorithm: {bellman_time:.4f} seconds")
